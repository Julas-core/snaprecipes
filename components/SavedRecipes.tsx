...
const SavedRecipes: React.FC<SavedRecipesProps> = ({ recipes, onSelect, onDelete }) => {
  ...
  return (
    <div className="w-full mt-8 animate-fade-in">
      <h3 className="text-xl font-serif font-bold text-amber-900 mb-4 px-2">Saved Recipes</h3>
      <div className="w-full overflow-x-auto pb-6 [scrollbar-width:thin] [scrollbar-color:#f59e0b_#fef3c7] [&::-webkit-scrollbar]:h-2.5 [&::-webkit-scrollbar-track]:bg-amber-100 [&::-webkit-scrollbar-track]:rounded-full [&::-webkit-scrollbar-thumb]:rounded-full [&::-webkit-scrollbar-thumb]:bg-amber-500 hover:[&::-webkit-scrollbar-thumb]:bg-amber-600">
        <div className="flex gap-4 px-2" style={{ minWidth: 'min-content' }}>
            {recipes.map((recipe) => (
              <div 
                key={recipe.id || recipe.recipeName}
                onClick={() => onSelect(recipe)}
                className="relative flex-shrink-0 w-[calc(33.333%-11px)] min-w-[300px] aspect-video bg-amber-200 rounded-xl shadow-lg overflow-hidden group cursor-pointer hover:shadow-xl transition-all hover:scale-[1.01]"
              >
                  {/* Image Background */}
                  {recipe.imageUrl ? (
                      <img src={recipe.imageUrl} alt={recipe.recipeName} className="w-full h-full object-cover" />
                  ) : (
                      <div className="w-full h-full flex items-center justify-center bg-amber-100 text-amber-800 font-serif">
                          No Image
                      </div>
                  )}
